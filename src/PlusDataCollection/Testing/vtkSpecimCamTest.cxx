...
int main(int argc, char **argv)
{
  ...
  if ( !args.Parse() )
  {
    ...
    std::cout << "\n\nvtkSpecimCamTest help:" << args.GetHelp() << std::endl;
    exit(EXIT_FAILURE);
  }
  ...
  vtkSmartPointer<vtkPlusSpecimCam> SpecimCam = vtkSmartPointer<vtkPlusSpecimCam>::New();

  SpecimCam->CreateDefaultOutputChannel();
...
// Add an observer to warning and error events for redirecting it to the stdout 
  ...
  SpecimCam->AddObserver("WarningEvent", callbackCommand); 
  SpecimCam->AddObserver("ErrorEvent", callbackCommand); 
  
  LOG_INFO("Initialize..."); 
  SpecimCam->Connect();

  if ( SpecimCam->GetConnected() )
  {
    LOG_INFO("Start recording..."); 
    SpecimCam->StartRecording(); 
  }
  else
  {
    SpecimCam->Disconnect();
    LOG_ERROR( "Unable to connect to FLIR Systems Spinnaker device"); 
    exit(EXIT_FAILURE); 
  }

  LOG_INFO("Stop recording...");
  SpecimCam->StopRecording(); 
  SpecimCam->Disconnect();
